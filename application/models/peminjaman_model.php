...
class peminjaman_model extends CI_model{
    ...
    public function insert($data,$buku_id)
    {
        ...
        $this->db->where('kode_buku',$buku_id); // FIX
        $this->db->update('buku');
    }

    public function get_detail($id) // FIX parameter
    {
        $this->db->select('detail_peminjaman.*,buku.judul_buku');        
        $this->db->from('detail_peminjaman');
        $this->db->join('buku','buku.kode_buku = detail_peminjaman.buku_id'); // FIX
    ...
    public function pengembalian($id)
    {
        ...
        // update stok
        $this->db->set('stok','stok + 1', FALSE);
        $this->db->where('kode_buku',$detail->buku_id); // FIX
        $this->db->update('buku');
    }
}